...
import { getLocalReceivedCards } from '../lib/localReceivedCards';

const FALLBACK_CARD_IMAGE = 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?auto=format&fit=crop&w=800&q=80';

const mergeWithLocalReceivedCards = (cards = []) => {
    const localCards = getLocalReceivedCards();
    if (!localCards.length) return cards;
    return [...cards, ...localCards]
        .sort((a, b) => new Date(b.date) - new Date(a.date));
};

const buildInitialCards = () => mergeWithLocalReceivedCards(
...
const Exchange = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = subscribeToUserCards(
            ...
            (remoteCards) => {
                ...
                const merged = normalizedCards.length
                    ? mergeWithLocalReceivedCards(normalizedCards)
                    : buildInitialCards();
                setCards(merged);
                setCardsLoading(false);
            },
    ...
    const handleResumeDraftTap = () => {
        if (user) {
            setHasPendingWrite(false);
            setViewMode('sender');
            return;
        }
        requestAction('sender');
    };
                ...
                >
                    ...
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                        ...
                        <Typography variant="body2" sx={{ opacity: 0.9 }}>
                            Tap to jump back in and finish embedding.
                        </Typography>
                    </Box>
                </Paper>
            )}
